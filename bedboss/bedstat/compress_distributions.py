...
def compress_region_distribution(raw: dict) -> Optional[dict]:
    ...
    for chrom, regions in raw.items():
        if not regions:
            result[chrom] = []
            continue
        rids = np.array([r.get("rid", 0) if isinstance(r, dict) else 0 for r in regions], dtype=np.int32)
        counts_arr = np.array([r.get("n", 1) if isinstance(r, dict) else 1 for r in regions], dtype=np.int32)
